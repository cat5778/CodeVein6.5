	...
	LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
	...
	pMesh->GetDeclaration(Decl); // 메쉬 정점 안에 들어있는 fvf 정보를 얻어오는 함수
	...
	if (0 != NumMaterials)
	{
		...
		for (_ulong i = 0; i < pDerivedMeshContainer->NumMaterials; ++i)
		{
			...
			if (!LoadNormalTexture(szFullPath, &pDerivedMeshContainer->ppNormalTexture[i]))
			{
				pDerivedMeshContainer->ppNormalTexture[i] = nullptr;
			}
		}
	}
	else
...
_bool CHierarchyLoader::LoadNormalTexture(const _tchar * szFilePath, LPDIRECT3DTEXTURE9 * ppTexture)
{
